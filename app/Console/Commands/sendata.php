...
class sendata extends Command
{
    ...
    public function handle()
    {
        ...
        if ($logtransfer) {
            ...
            $logTimestamp = Carbon::parse($logtransfer->timestamp);
            $logInterval = $logTimestamp->copy()->addMinutes($interval);
            if ($currentDateTime->lte($logInterval)) {
                echo "The current date and time is within the interval!";
            } else {
                echo "The current date and time is outside the interval.";
                // $remainingSeconds = $currentDateTime->diffInSeconds($logInterval, false);
