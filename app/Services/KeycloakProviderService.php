...
use App\Models\KeycloakUser as User;
use GuzzleHttp\RequestOptions;

class KeycloakService extends AbstractProvider
{
    ...
    {
        return rtrim(rtrim(config('services.keycloak.base_url'), '/') . '/auth/realms/' . config('services.keycloak.realms'), '/');
    }
    ...
    public function user()
    {
        ...
        $user->setToken(Arr::get($response, 'access_token'));
        $user->refreshToken = Arr::get($response, 'refresh_token');
        $user->expiresIn = Arr::get($response, 'expires_in');
        $user->setIdToken(Arr::get($response, 'id_token'));
