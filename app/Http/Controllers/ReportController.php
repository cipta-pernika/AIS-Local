...
class ReportController extends Controller
{
    public function konsolidasi(Request $request)
    {
        ...
        $pandu_tidak_terjadwal_data = PanduTidakTerjadwal::whereBetween(DB::raw('DATE(created_at)'), [$startDateTime, $endDateTime])
            ->whereIn('ais_data_vessel_id', $intersect_ids)->where('isPassing', 0)
            ->get();
        ...
        $total_pandu = $summaryData['pandu_count'] + $total_pandu_tidak_tejadwal + $total_late_pandu + $intersect_ids_count;
        $total_muat = $summaryData['bongkar_muat_count'] + $total_tidak_terjadwal_bongkar + $total_late_bongkar + $intersect_ids_count;
