...
<p><strong>Perimeter : </strong><br>_p_perimetro</p>`,
        ...
        msj_disable_tool: 'Apakah Anda ingin menonaktifkan?',
    ...
    _UpdateAreaPerimetro(layer) {
        ...
        const areaValue = `${area.toLocaleString('en-US', options)} m²`;
        const perimeterValue = `${perimeter.toLocaleString('en-US', options)} m`;
    ...
    _toggleMeasure() {
        if (this.options.checkonedrawpoligon) {
            ...
            const respuesta = confirm(this.options.msj_disable_tool);
            ...
            if (respuesta) {
                ...
                localStorage.removeItem('openMeasure')
            }
            ...
            // this._measurePanel.addTo(map);
            localStorage.setItem("openMeasure", '1');
            this._measureHandler.enable();
